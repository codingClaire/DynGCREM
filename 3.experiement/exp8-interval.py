...
SAMPLE_SIZE = 40000
RANDOM_SEED = 2019
DAYS= 15 # set DAYS to be 15,30,45,and 60 to finish dynamic graph interval experiement
HOURS, MINUTES, SECOND=24, 60, 60
...
def GCN_LSTM_classification(modelFileName,rs):
    ...
    for i in range(subSum-1,subSum):
        ...
        result = lgb_train_model_with_split(
            pd.DataFrame(trainX_2D), pd.DataFrame(trainY_1D), rs)
        print_res(result)
        return result
...
def GCN_classfication(rs):
    ...
    inputX=trainX_2D
    result = lgb_train_model_with_split(
        pd.DataFrame(inputX), pd.DataFrame(trainY_1D), rs)
    print_res(result)
    return result
    
def pure_feature_classification(i):
    trainX, trainY, testX, testY = get_pure_feature(False)
    trainX_2D, trainY_1D = trainX[:, subSum-1, :], trainY[:, subSum-1, 0]
    result = lgb_train_model_with_split(
        pd.DataFrame(trainX_2D), pd.DataFrame(trainY_1D), 2011+i)
    print_res(result)
    return result
...
idx=1
for rs in top5:
    #GCN_tol_embedding(rs)
    #modelName="/model_trtr_t100_i%d_%d" %(DAYS,rs)
    #print(modelPathName+modelName)
    #RNN_model(modelPathName,modelName)
    print("GCN_LSTM_classification:")
    modelName="/model_trtr_t100_i%d_%d.h5" %(DAYS,rs) 
    mres=GCN_LSTM_classification(modelPathName+modelName,2011)
    idx+=1
    m.append(mres)
    print("GCN classification:")
    m1res=GCN_classfication(rs)
    m1.append(m1res)
