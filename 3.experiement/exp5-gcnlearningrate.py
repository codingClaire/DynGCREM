...
def GCN_classfication(learningrate):
    ...
    for i in range(rcnt):
        for idx in range(subSum-1, subSum):
            ...
            embSize = 8
            fGCNembedding = get_GCN_embedding(epoch=6, lr=learningrate, weight_decay=1e-6,
...
lr_res = []
for lr in [0.001,0.005,0.01,0.015,0.02,0.025,0.03,0.035,0.04,0.045,0.05]:
    print(lr)
    tree_res = GCN_classfication(lr)
    lr_res.append(tree_res)
    print(tree_res)
    
df_res = pd.DataFrame(lr_res)
df_res.head()