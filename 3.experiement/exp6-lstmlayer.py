...
def RNN_model_2layer(savePath,saveName):
    ###### model setting ######
    batchSize = 128
    epochsNum = 10
...
def RNN_model_1layer(savePath,saveName):
    ###### model setting ######
    batchSize = 128
    epochsNum = 10
...
def RNN_model_3layer(savePath,saveName):
    ###### model setting ######
    batchSize = 128
    epochsNum = 10
...
def RNN_model_4layer(savePath,saveName):
    ###### model setting ######
    batchSize = 128
    epochsNum = 10
    ...
    ###### model structure ######
    model = Sequential()
    model.add(LSTM(units=48, activation='tanh', return_sequences=True,
                   input_shape=(timestamp, featuresNum)))
    model.add(LSTM(units=32, activation='relu', return_sequences=True,
                   input_shape=(timestamp, featuresNum)))
    model.add(LSTM(units=16, activation='tanh', return_sequences=True,
...
for i in top5[1:]:
    ...
    m1.append(m1res)
    # 4 layer
    modelName="/model_trtr_t50_%d_4layer" %i
    RNN_model_4layer(modelPathName,modelName)
    print(modelPathName+modelName)
    modelName="/model_trtr_t50_%d_4layer.h5" %i
    # 3 layer
    modelName="/model_trtr_t50_%d_3layer" %i
    RNN_model_3layer(modelPathName,modelName)
    print(modelPathName+modelName)
    modelName="/model_trtr_t50_%d_3layer.h5" %i
    # 2 layer
    modelName="/model_trtr_t50_%d_2layer" %i
    RNN_model_2layer(modelPathName,modelName)
    print(modelPathName+modelName)
    modelName="/model_trtr_t50_%d_2layer.h5" %i
    # 1 layer
    modelName="/model_trtr_t50_%d_1layer" %i
    RNN_model_1layer(modelPathName,modelName)
    print(modelPathName+modelName)
    modelName="/model_trtr_t50_%d_1layer.h5" %i
    
    print("GCN_LSTM_classification for 4 layer:")
...
print((pd.DataFrame(m).sum()-pd.DataFrame(m).max()-pd.DataFrame(m).min())/6)
print((pd.DataFrame(m1).sum()-pd.DataFrame(m1).max()-pd.DataFrame(m1).min())/6)
