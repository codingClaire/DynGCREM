...
tolFeaturePathName = interPathName + "/tol_features"
embPathName = "./embeddings/graph_%d/interval_%d" % (SAMPLE_SIZE, INTERVAL)
#GCNPathName = embPathName+"/GCNembedding"
GCNPathName= embPathName+"/GCNembedding_test"
LINEPathName = embPathName+"/LINE"
...
def RNN_model(savePath,saveName):
    ...
    lr_reduce = ReduceLROnPlateau(
        monitor='loss', factor=0.1, epsilon=0.0001, patience=1, verbose=1)
...
#show_MSE(modelPathName+"/model.h5")
...
# feature for LSTM autoencoder
def RNN_only_model(savePath,saveName):
    ###### model setting ######
    batchSize = 128
    epochsNum = 20
    ...
    trainX, trainY, testX, testY = get_input_data(
        GCNPathName+"%d_%d" %(embSize,testNum), "/fGCNembedding", True, True) 
    #False means not split the dataset
    trainX,testX= trainX[:, :, 8:],testX[:,:,8:] 
    # 没注释表示只选取单纯特征进行训练
    print(trainX.shape, trainY.shape)
    featuresNum = trainX.shape[2]
    timestamp = trainX.shape[1]
...
print("warning:only for LSTM")
if not os.path.exists(modelPathName):
    os.makedirs(modelPathName)
modelName="/model_trtr_onlylstm"
print(modelPathName+modelName)
RNN_only_model(modelPathName,modelName)
print(modelPathName+modelName)

modelName="/model_trtr"
RNN_model(modelPathName,modelName)


# ablation experiement
print("GCN_LSTM_classification:")
GCN_LSTM_classification(modelPathName+"/model_trtr.h5")
print("GCN classification:")
GCN_classfication()
print("LSTM classification:")
LSTM_classification(modelPathName+"/model_trtr_onlylstm.h5")
print("pure feature classification:")
pure_feature_classification()
if not os.path.exists(modelPathName):
    os.makedirs(modelPathName)
